...
import { getFastAPI, type Instructor, type CreateInstructorRequest } from '../api/school'
...
export default function InstructorsSection() {
  ...
  const createMutation = useMutation({
    mutationFn: (req: CreateInstructorRequest) => api.createInstructorInstructorsPost(req),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['instructors'] }); setShowAdd(false) },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: CreateInstructorRequest }) =>
      api.updateInstructorInstructorsIdPatch(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['instructors'] }); setEditing(null) },
  })

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.deleteInstructorInstructorsIdDelete(id),
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState label="No instructors yet" />
      ) : (
        <>
          {/* Desktop Table View */}
                ...
                {instructors.map((inst) => (
                  <>
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-[14px] text-[#374151] dark:text-[#d4d4d8] transition-colors">{inst.email}</td>
                      <td className="px-5 py-3.5 text-[14px] text-[#6b7280] dark:text-[#a1a1aa] transition-colors">#{inst.id}</td>
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <ActionBtn onClick={() => toggleExpand(inst.id!)} label="Courses" icon="courses" />
                          <ActionBtn onClick={() => setEditing(inst)} label="Edit" icon="edit" />
                          <ActionBtn
                            onClick={() => { if (confirm('Delete this instructor?')) deleteMutation.mutate(inst.id!) }}
                            label="Delete"
                            icon="delete"
                            danger
                          />
                    ...
                    {expandedId === inst.id && (
                      <CoursesRow key={`inst-courses-${inst.id}`} instructorId={inst.id!} />
                    )}
                  </>
                ))}
              </tbody>
            ...
            {instructors.map((inst) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12px] text-[#6b7280] dark:text-[#a1a1aa] transition-colors">#{inst.id}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <span className="text-[14px] text-[#374151] dark:text-[#d4d4d8] font-medium break-all transition-colors">{inst.email}</span>
                </div>

                {expandedId === inst.id && (
                  <div className="mb-4 border-t border-[#f3f4f6] dark:border-[#1c1c1e] pt-3">
                    <span className="text-[12px] text-[#6b7280] dark:text-[#a1a1aa] block mb-2 font-medium transition-colors">Assigned Courses</span>
                    <CoursesRowMobile instructorId={inst.id!} />
                  </div>
                )}
                ...
                <div className="flex items-center justify-between border-t border-[#f3f4f6] dark:border-[#1c1c1e] pt-3">
                  ...
                  >
                    ...
                    {expandedId === inst.id ? 'Hide Courses' : 'Courses'}
                  </button>
                  <div className="flex items-center gap-1.5">
                    <ActionBtnMobile onClick={() => setEditing(inst)} label="Edit" icon="edit" />
                    <ActionBtnMobile
                      onClick={() => { if (confirm('Delete this instructor?')) deleteMutation.mutate(inst.id!) }}
                      label="Delete"
                      icon="delete"
      ...
      {showAdd && (
        <Modal title="Add Instructor" onClose={() => setShowAdd(false)}>
          <PersonForm onSubmit={handleAdd} loading={createMutation.isPending} submitLabel="Add Instructor" />
        </Modal>
      )}

      {editing && (
        <Modal title="Edit Instructor" onClose={() => setEditing(null)}>
          <PersonForm
            onSubmit={handleEdit}
...
function CoursesRow({ instructorId }: { instructorId: number }) {
  const api = getFastAPI()
  const { data, isLoading } = useQuery({
    queryKey: ['instructor-courses', instructorId],
    queryFn: () => api.getInstructorCoursesInstructorsIdCoursesGet(instructorId),
  })
    ...
    <tr className="bg-[#f8f9fa] dark:bg-[#18181b] transition-colors">
      <td colSpan={4} className="px-5 py-3">
        {isLoading ? (
          ...
          <span className="text-[13px] text-[#6b7280] dark:text-[#a1a1aa]">No courses assigned.</span>
        ) : (
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <Field label="First Name" name="first_name" placeholder="Dr. Alice" defaultValue={defaultValues?.first_name} required />
        <Field label="Last Name" name="last_name" placeholder="Johnson" defaultValue={defaultValues?.last_name} required />
      </div>
      <Field label="Email" name="email" type="email" placeholder="alice@school.edu" defaultValue={defaultValues?.email} required />
      <button
        type="submit"
        disabled={loading}
...
const PASTEL = ['#bfdbfe', '#bbf7d0', '#fde68a', '#fecaca', '#e9d5ff', '#fed7aa']
...
function CoursesRowMobile({ instructorId }: { instructorId: number }) {
  const api = getFastAPI()
  const { data, isLoading } = useQuery({
    queryKey: ['instructor-courses', instructorId],
    queryFn: () => api.getInstructorCoursesInstructorsIdCoursesGet(instructorId),
  ...
  if (!data || Object.keys(data).length === 0) return <span className="text-[13px] text-[#6b7280] dark:text-[#a1a1aa]">No courses assigned.</span>
